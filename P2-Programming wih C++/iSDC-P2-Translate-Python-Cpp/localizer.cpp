...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
    ...
    float area = Row_Grid * Column_Grid;
    float belief_per_cell = 1.0 / area;
    vector < vector<float> > newGrid (Row_Grid, vector <float>(Column_Grid, belief_per_cell));
  ...
  float blurring) 
{
  ...
  // your code here
  int Row_Beli = beliefs.size();
  int Column_Beli = beliefs[0].size();
  int new_i = 0, new_j = 0;
  vector <vector<float> > newGrid (Row_Beli, vector <float>(Column_Beli));

  for (int row = 0; row < Row_Beli; row++) {
      for (int column = 0; column < Column_Beli; column++) {
          new_i = (row + dy) % Row_Beli;
          new_j = (column + dx) % Column_Beli;
...
//	vector< vector <float> > newGrid;
	...
	// your code here
  	int Row_Beliefs = grid.size();
    int Column_Beliefs = grid[0].size();
    vector< vector <float> > newGrid(Row_Beliefs,vector <float>(Column_Beliefs) );
