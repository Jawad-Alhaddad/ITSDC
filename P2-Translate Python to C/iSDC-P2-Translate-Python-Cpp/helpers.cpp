...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	float d = 0 ;//it will hold the sum
  	int r = grid.size(); // short for row
  	int c = grid[0].size();//short for column
	vector<float> row;

	// todo - your code here
	// 1) let us get the total sum
  	 for (int i =0; i<r;i++){	
       for (int j =0; j<c;j++){	
     	d+=grid[i][j];
        } 
	...
	// 2) divide all matrix elements by determinant
  	for (int i =0; i<r;i++){	
      row.clear();
      for (int j =0; j<c;j++){	
      	row.push_back(grid[i][j]/d);  
      } 
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	int Column_Grid = grid[0].size();
	float center_prob = 1.0 - blurring;
	float corner_prob = blurring / 12.0;//blurring*(2/3)*(1/4)
	float adjacent_prob = blurring / 6.0;//blurring*(1/3)*(1/4)
	int new_r ;//new row
	int new_c ;// new column
	vector < vector <float> > newGrid (Row_Grid,vector <float>(Column_Grid));
	...
	for (int i = 0; i < Row_Grid; i++) {
		for (int j = 0; j < Column_Grid; j++) {
			for (int dx = -1; dx < 2; dx++) {
				for (int dy = -1; dy < 2; dy++) { 
					new_r = (Row_Grid + i + dy) % Row_Grid;
					new_c = (Column_Grid + j + dx) % Column_Grid;
					newGrid[new_r][new_c] += window[dx+1][dy+1] * grid[i][j];					
				}			    	
			}
		}
	}
